...
void	handle_syntax_errors(t_lex_list *token)
{
	...
	while (token)
	{
		if (is_special_for_specials(token->a_type) && token->next && is_special_for_specials(token->next->a_type))
			put_syntax_error();
...
int main(void)
{
    ...
    t_test tests[] = {
        ...
        // ===== SEMICOLON TESTS (IF SUPPORTED) =====
        // Assuming minishell does NOT support ; syntax initially, so mark as errors (expected=1)
        // Change expected to 0 if your minishell *does* support them.
        { "Semicolon Basic", "echo hello; echo world", 1, "Commands separated by semicolon (mark as error if unsupported)" },
        { "Semicolon Multiple", "echo hello; echo world; echo test", 1, "Multiple commands separated by semicolons (mark as error if unsupported)" },
        { "Semicolon With Pipe", "echo hello | grep hello; echo world", 1, "Semicolon with pipe (mark as error if unsupported)" },
        { "Semicolon With Redirection", "echo hello > file.txt; cat file.txt", 1, "Semicolon with redirection (mark as error if unsupported)" },
        { "Semicolon Empty", "echo hello;;echo world", 1, "Empty command with semicolon (syntax error even if supported)" },
        { "Semicolon Start", "; echo hello", 1, "Semicolon at start (syntax error)"},
        { "Semicolon End", "echo hello;", 1, "Semicolon at end (syntax error if not interactive)"}, // Behavior might differ interactive vs script
        { "Semicolon Pipe Mix", "echo a; ls | wc ; echo b", 1, "Mixed semicolon and pipe"},
        { "Semicolon Redir Mix", "echo a > file ; cat file", 1, "Mixed semicolon and redirection"},
        { "Semicolon Logic Mix", "echo a && echo b ; echo c", 1, "Mixed semicolon and logic"}, // Only if both are supported
