...
t_ast_tree *parse_parenthesis_cmds(t_lex_list **token)
{
	...
	if (*token && ((*token)->a_type == APPEND || (*token)->a_type == HEREDOC
	|| (*token)->a_type == IN_REDIR || (*token)->a_type == OUT_REDIR))
	{
		create_redirection((*token)->a_type, (*token)->next);
	}
	return NULL;
}
...
int	main(void)
{
	...
	token = lexing_the_thing("ls | hello | cat && echo ");
	token2 = token;
