...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/29 22:59:45 by jhamdaou          #+#    #+#             */
/*   Updated: 2025/05/29 23:05:06 by moel-amr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_env_list				*find_key_node(t_env_list *current, char *key,
							t_env_list **prev);
...
int						check_operator_before_close_paren(t_lex_list *token,
							int *status);
int						check_pipe_after_open_paren(t_lex_list *token,
							int *status);
...
void					do_the_heredoc(t_redirect *redir, int pipe_fd[2],
							t_env_list *env);
...
void					expand_norminette_3(t_expd_norm *expd, t_expd2 *expd2,
							t_ast_tree *astree);
int						expand_norm4(t_expd_norm *expd, t_expd2 *expd2,
							t_ast_tree *astree, t_env_list *env);
...
void					file_expand_norm2(t_file_expd *fxpd, t_expd2 *expd2,
							t_redirect *redir, t_env_list *env);
void					file_expand_norm3(t_file_expd *fxpd, t_expd2 *expd2,
							t_redirect *redir);
int						file_expand_norm4(t_file_expd *fxpd, t_expd2 *expd2,
							t_redirect *redir, t_env_list *env);
void					file_expand_norm5(t_file_expd *fxpd, t_expd2 *expd2);
void					file_expand_norm6(t_file_expd *fxpd, t_expd2 *expd2,
							t_redirect *redir);
...
void					handle_single_wild_card2(t_wild_norm *wild,
							char *current_arg, int old_is_space,
							int old_q_type);
void					handle_wild_card(t_ast_tree *node);
void					wild_card_norm1(t_wild_norm *wild, char *current_arg,
							int old_q_type, int point_flag);
int						wild_card_norm2(char *current_arg);
char					**wild_card_norm3(char *current_arg, int reads_counter);
void					wild_card_norm4(t_wild_norm *wild, char **all_dirs,
							int reads_counter, int point_flag);
void					wild_card_norm5(t_wild_norm *wild, char *current_arg,
...
void					init_norm(t_norm_m *mainn, int ac, char **av,
							t_ast_tree **astree);
void					is_tty(char **input);
void					is_tt2(t_env_list **env);
void					exec_tree_ndrest(t_ast_tree **astree, int *status,
							t_env_list **envv, char **input);
int						handle_token(t_lex_list **tokens, char **input,
							int *status);
