...
static void I_HATE_EXPANDING_FILE(t_redirect *redir,t_env_list *env, int status)
{
    int k = 0;
	char **file_name = NULL;
	int size = 0;
    ...
    {
        expanded_for_single_word(&file_name,redir->file_name[k],env,  status,redir->is_space[k],k,&size,redir->q_types[k],&is_space);
        k++;
    }
    k = 0;
	while (k < redir->file_str_count)
